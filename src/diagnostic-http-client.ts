...
import type { DiagnosticComponentProfile } from "./types.js";
import type {
  AcceptedPayload,
  BridgeEnvelope,
  ConversationPayload,
...
type DiagnosticClientConfig =
  | Pick<AppConfig["diagnosticBridge"], "baseUrl" | "token" | "caller" | "timeoutMs">
  | Pick<DiagnosticComponentProfile, "baseUrl" | "token" | "caller" | "timeoutMs">;

export class DiagnosticHttpClient implements DiagnosticGateway {
  constructor(private readonly config: DiagnosticClientConfig) {}
  ...
  private async request<T>(method: string, path: string, body?: Record<string, unknown>): Promise<BridgeEnvelope<T>> {
    ...
    try {
      ...
      if (error instanceof Error && error.name === "AbortError") {
        await delay(0);
        throw new Error(`Diagnostic component request timeout after ${this.config.timeoutMs}ms`);
      }
      throw error;
    } finally {
...
export class SmartKitClient extends DiagnosticHttpClient {}
